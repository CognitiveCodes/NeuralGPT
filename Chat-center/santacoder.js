...
async function askQuestion(question) {
  try {
    ...
    // Send a question to the chatbot and display the responseasync function askQuestion(question) {
    const response = await fetch("https://bigcode-santacoder-demo.hf.space/run/predict", {
      method: "POST",
    ...
    // Check if the response data contains the "data" field and it has at least one element
    if (data.data && data.data.length > 0) {
      const outputText = data.data[0];
      
      // Decode the outputText from the JSON format
      const decodedOutputText = decodeURIComponent(JSON.parse(`"${outputText}"`));
      return decodedOutputText;
    } else {
      console.error("Invalid response format from the chatbot API.");
      return null;
    }
  } catch (error) {
    console.error(error);
