...
wss.on('connection', (ws) => {
  ...
  ws.send('Hello! Can you integrate yourself with the local sql database and file system?');
  ...
  ws.on('message', async (message) => {
    ...
    try {
      ...
      if (parsedMessage.text) { // <-- Check for the presence of the "text" property
        const answer = await processUserInput(parsedMessage.text); // <-- Pass the "text" property to the askQuestion function
